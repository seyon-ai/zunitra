...
export function watchDisplay(w, size) {
  ...
  // Collect candidate URLs, filter out ibb.co page links
  var rawUrls = [];
  ...
  for (var i = 0; i < rawUrls.length; i++) {
    ...
    if (!u) continue;
    // ibb.co/XXXXX = share page, skip. i.ibb.co = direct image, keep.
    if (u.match(/^https?:\/\/ibb\.co\/[^/]+$/)) continue;
    imgUrl = u;
    break;
  }

  // No image — return SVG illustration
  if (!imgUrl) return watchSVG(w, size);

  // Has image — return img element
  // Simple and reliable. No onerror complexity.
  var h = Math.round(size * 1.2);
  return '<img src="' + imgUrl.replace(/"/g, "%22") + '" '
       + 'alt="' + (w.name || "").replace(/"/g, "") + '" '
