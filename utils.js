...
export function watchDisplay(w, size) {
  ...
  // Collect + validate URLs
  var rawUrls = [];
  if (w.images && w.images.length) rawUrls = w.images.slice();
  else if (w.imageUrl) rawUrls = [w.imageUrl];

  var urls = rawUrls.map(function(u) {
    u = (u || "").trim();
    if (!u) return "";
    // ibb.co/XXXXX is a share page, NOT a direct image — skip it
    if (/^https?:\/\/ibb\.co\/[^/]+$/.test(u)) return "";
    return u;
  }).filter(Boolean);

  // No valid URL — return SVG
  if (!urls.length) return watchSVG(w, size);

  // Use img element. On error replace entire wrapper with SVG.
  // Store SVG as a JS string in a global registry keyed by product id.
  var pid = (w.id || "p" + Math.random().toString(36).slice(2));
  var h   = Math.round(size * 1.2);

  // Register SVG fallback in global store (set during render)
  if (typeof window !== "undefined") {
    window.__zunSVG = window.__zunSVG || {};
    window.__zunSVG[pid] = watchSVG(w, size);
  }
  ...
  return '<img src="' + urls[0] + '" '
       ...
       + 'onerror="(function(el){var s=(window.__zunSVG&&window.__zunSVG[el.dataset.pid])||el.outerHTML;el.outerHTML=s;})(this)"'
       + '/>';
}
