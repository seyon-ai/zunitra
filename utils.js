...
export function watchDisplay(w, size = 200) {
  // Convert ibb.co page links to direct image links automatically
  let imgUrl = w.imageUrl || w.images?.[0] || "";
  if (imgUrl) {
    // Fix ibb.co page URLs -> direct image URLs
    imgUrl = imgUrl
      .replace("https://ibb.co/", "https://i.ibb.co/")
      .replace("http://ibb.co/", "https://i.ibb.co/");
    return `<img src="${imgUrl}" alt="${w.name || ''}" 
      style="width:${size}px;height:${size * 1.2}px;object-fit:contain;display:block"
      onerror="this.outerHTML=window._watchSVGFallback('${w.id||'w'}','${w.color||'#111'}','${w.accentColor||'#C9A84C'}',${size})">`;
  }
  return watchSVG(w, size);
}
...
export function fmtPrice(n) {
  ...
  return `$${usd.toLocaleString("en-US")} <span class="inr-price">/ ₹${inr.toLocaleString("en-IN")}</span>`;
}
...
// Global SVG fallback for img onerror
window._watchSVGFallback = function(id, color, accent, size) {
  // Create a simple inline watch SVG as fallback
  return `<svg viewBox="0 0 200 240" width="${size}" height="${size*1.2}" xmlns="http://www.w3.org/2000/svg">
    <rect x="78" y="2" width="44" height="50" rx="7" fill="${color}" stroke="${accent}" stroke-opacity="0.3" stroke-width="1"/>
    <circle cx="100" cy="120" r="70" fill="${color}" stroke="${accent}" stroke-width="2.5"/>
    <circle cx="100" cy="120" r="58" fill="none" stroke="${accent}" stroke-opacity="0.2" stroke-width="0.5"/>
    <text x="100" y="102" text-anchor="middle" font-family="Georgia,serif" font-size="7" fill="${accent}" letter-spacing="2">ZUNITRA</text>
    <line x1="100" y1="120" x2="86" y2="88" stroke="white" stroke-width="3" stroke-linecap="round"/>
    <line x1="100" y1="120" x2="124" y2="78" stroke="${accent}" stroke-width="2" stroke-linecap="round"/>
    <circle cx="100" cy="120" r="4" fill="${accent}"/>
    <rect x="78" y="188" width="44" height="50" rx="7" fill="${color}" stroke="${accent}" stroke-opacity="0.3" stroke-width="1"/>
  </svg>`;
};
