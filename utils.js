...
export function watchDisplay(w, size) {
  size = size || 200;
  var urls = [];
  if (w.images && w.images.length) urls = w.images;
  else if (w.imageUrl) urls = [w.imageUrl];
  // Fix ibb.co page links to direct links
  urls = urls.map(function(u){
    return u.replace("https://ibb.co/","https://i.ibb.co/").replace("http://ibb.co/","https://i.ibb.co/");
  }).filter(Boolean);
  if (urls.length) {
    return '<img src="'+urls[0]+'" alt="'+(w.name||'')+'" '
      +'style="width:'+size+'px;height:'+(size*1.2)+'px;object-fit:contain;display:block;margin:auto" '
      +'onerror="this.style.display=\'none\';this.nextElementSibling.style.display=\'block\'" />'
      +'<div style="display:none">'+watchSVG(w,size)+'</div>';
  }
